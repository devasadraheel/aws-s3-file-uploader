...
export class FilesController {
  ...
  async presignUpload(@Body() presignUploadDto: PresignUploadDto) {
    try {
      ...
      const url = await this.s3Service.getPresignedPutUrl(
        key,
        contentType,
        FILE_UPLOAD_CONFIG.PRESIGNED_URL_EXPIRY_SECONDS,
      );
      ...
      throw new HttpException(
        ERROR_MESSAGES.UPLOAD_URL_FAILED,
  ...
  async presignDownload(@Query() presignDownloadDto: PresignDownloadDto) {
    try {
      ...
      throw new HttpException(
        ERROR_MESSAGES.DOWNLOAD_URL_FAILED,
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async getFileMetadata(@Query() presignDownloadDto: PresignDownloadDto) {
    try {
      ...
      throw new HttpException(
        ERROR_MESSAGES.METADATA_FAILED,
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  private validateFileUpload(
    ...
    if (!key.startsWith(FILE_UPLOAD_CONFIG.KEY_PREFIX)) {
      throw new BadRequestException(
        `Key must start with "${FILE_UPLOAD_CONFIG.KEY_PREFIX}"`,
      );
    }
